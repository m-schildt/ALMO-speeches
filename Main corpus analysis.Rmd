...
```{r}
library("quanteda")
main_corpus <- corpus(sample_df2, docnames= "Date", text_field = "Text")
summary(main_corpus)
...
dfm_covid_2020 <- tokens(subset_president_2020,include_docvars = TRUE) %>% tokens_select(covid_dic) %>% dfm() 


docnames(dfm_covid_2020) <- docvars(dfm_covid_2020, "Date")
dfm_covid_2020

covid_2020_df <- convert(dfm_covid_2020, to = "data.frame")
